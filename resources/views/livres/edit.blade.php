<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Modifier le livre : ') }} {{ $livre->titre }}
        </h2>
    </x-slot>
                ...
                <div class="p-6 text-gray-900">
                    ...
                    <form action="{{ route('livres.update', $livre) }}" method="POST" class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="titre" name="titre" type="text" class="mt-1 block w-full" :value="old('titre', $livre->titre)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('titre')" />
                        </div>

                        <div>
                            <x-input-label for="annee_publication" :value="__('Année de publication')" />
                            <x-text-input id="annee_publication" name="annee_publication" type="number" class="mt-1 block w-full" :value="old('annee_publication', $livre->annee_publication)" required />
                            <x-input-error class="mt-2" :messages="$errors->get('annee_publication')" />
                        </div>

                        <div>
                            <x-input-label for="nombre_pages" :value="__('Nombre de pages')" />
                            <x-text-input id="nombre_pages" name="nombre_pages" type="number" class="mt-1 block w-full" :value="old('nombre_pages', $livre->nombre_pages)" required />
